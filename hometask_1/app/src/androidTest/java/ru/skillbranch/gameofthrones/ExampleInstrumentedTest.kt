...
class ExampleInstrumentedTest {
    ...
    private val stubHouseTargaryen =
        HouseRes(
    ...
    private val stubCharacterJonSnow =
        CharacterRes(
    ...
    private val stubCharacterRhaegar =
        CharacterRes(
    ...
    private val stubCharacterLyanna =
        CharacterRes(
    ...
    fun insert_house_and_drop_db() {
        //Запись в базу
        val lock0 = Object()
        RootRepository.insertHouses(
            listOf(stubHouseStark)
        ) {
    ...
    fun insert_characters_and_find() {
        ...
        val lock0 = Object()
        RootRepository.insertHouses(
            listOf(stubHouseStark, stubHouseTargaryen)
        ) {
            synchronized(lock0) { lock0.notify() }
        }
        synchronized(lock0) { lock0.wait() }
        ...
        RootRepository.insertCharacters(
            characters
        ) {
            synchronized(lock1) { lock1.notify() }
        }
        ...
        RootRepository.findCharactersByHouseName(
            "Stark"
        ) {
            actualCharacters = it
        ...
        assertEquals(
            stubCharacterJonSnow.name,
            actualCharacters?.first()?.name
        )
        assertEquals(
            stubCharacterJonSnow.aliases,
            actualCharacters?.first()?.aliases
        )

        assertEquals(
            stubCharacterLyanna.name,
            actualCharacters?.last()?.name
        )
        assertEquals(
            stubCharacterLyanna.aliases,
            actualCharacters?.last()?.aliases
        )
    }
    ...
    fun insert_characters_and_full() {
        ...
        RootRepository.findCharacterFullById(
            "583"
        ) {
            actualCharacter = it
            synchronized(lock2) { lock2.notify() }
        ...
        assertEquals(
            stubCharacterJonSnow.aliases,
            actualCharacter?.aliases
        )

        assertEquals(
            stubCharacterRhaegar.name,
            actualCharacter?.father?.name
        )
        assertEquals(
            "Targaryen",
            actualCharacter?.father?.house
        )

        assertEquals(
            stubCharacterLyanna.name,
            actualCharacter?.mother?.name
        )
    ...
    fun get_need_houses_with_characters() {
        ...
        assertEquals(
            "We Do Not Sow",
            actualHouses?.first()?.first?.words
        )
